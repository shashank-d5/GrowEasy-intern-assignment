...
import { CSVRow, CRMRecord, ALLOWED_CRM_STATUSES, ALLOWED_DATA_SOURCES, CRM_FIELD_ORDER } from '../types';

export function parseCSV(content: string, delimiter: string = ','): { headers: string[]; rows: CSVRow[] } {
  ...
  const rows: CSVRow[] = raw.map((row: any) => {
...
export function validateAndCleanRecords(records: any[]): { records: CRMRecord[]; skipped: number; errors: string[] } {
  ...
  for (let i = 0; i < records.length; i++) {
    ...
    const cleaned: CRMRecord = {} as CRMRecord;
